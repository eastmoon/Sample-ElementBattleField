...
package section.DEMO.Module.Demo.Space.variable
{
	...
	public class B2DBox extends B2DObject
	{
		/*const variable：常數變數*/
		...
		//
		public function B2DBox( a_space : IRMSpace,
		...
		public override function Initial( a_param : Object = null ) : void
		{
				...
				switch( paramStr )
				{
					...
					case "phy" :
						if( a_param[ paramStr ] is Array && ( a_param[ paramStr ] as Array ).length == 3 )
						{
							...
							fixtureDef.density = ( a_param[ paramStr ] as Array )[1];
							fixtureDef.restitution = ( a_param[ paramStr ] as Array )[2];
						}
					break;
				}
			}
			...
			{
				this.m_displauObject.width = sizeInfo.width; 
				this.m_displauObject.height = sizeInfo.height; 
			}
			*/
			// 建立物理型體
			shape.SetAsBox( sizeInfo.width / 2 / this.m_worldScale, sizeInfo.height / 2 / this.m_worldScale );
