		...
		{			
			...
			if( this.m_movePoint != null && this.m_movePoint.getY() != local.getY() && this.m_movePoint.getX() != local.getX() )
			{
				...
				if( Math.abs(distance) > Math.abs(diffX) )
					shiftX = this.m_movePoint.getX();
				else
					shiftX = local.getX() + distance;
				
				// 4. 計算移動的 Y 點
				var shiftY : Number = 0;
				distance = Math.sin( angle ) * this.m_speed * ((diffX >= 0) ? 1 : -1);
				if( Math.abs(distance) > Math.abs(diffY) )
					shiftY = this.m_movePoint.getY();
				else
					shiftY = local.getY() + distance;
					
				// 設定移動作標
				local.SetMouseJoint( shiftX, shiftY );
				//local.SetMouseJoint( this.m_movePoint.getX(), this.m_movePoint.getY() );
			}
			else if( this.m_movePoint != null && Math.floor(this.m_movePoint.getX()) == Math.floor(local.getX()) 
					&& Math.floor(this.m_movePoint.getY()) == Math.floor(local.getY()) )
			{
				local.RemoveMouseJoint();
