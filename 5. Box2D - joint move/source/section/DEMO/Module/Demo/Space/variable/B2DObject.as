...
package section.DEMO.Module.Demo.Space.variable
{
	...
	public class B2DObject extends RMSpaceVar
	{
		...
		public function SetMouseJoint( a_x : Number, a_y : Number ) : void
		{
			if( this.m_body == null && this.m_world == null )
				return ;
			
			/*MouseJoint 必須先建立關連要先以自身為移動目標後再設定移動目標*/
			if( this.m_mouseJoint == null )
			{
				...
				var jointDef : b2MouseJointDef= new b2MouseJointDef();
			 	...
			 	//jointDef.target = new b2Vec2( a_x / this.m_worldScale, a_y / this.m_worldScale );
				jointDef.target = new b2Vec2( this.getX() / this.m_worldScale, this.getY() / this.m_worldScale );
			 	jointDef.maxForce = 200 * this.m_body.GetMass();
			...
			if( this.m_mouseJoint != null )
			{
				trace( "JointM" , a_x - this.getX(), a_y - this.getY());
				// 存在，修改移動關連
				this.m_mouseJoint.SetTarget( new b2Vec2( a_x / this.m_worldScale, a_y / this.m_worldScale ) );
			}
		}
